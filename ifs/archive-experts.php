<?php
	...
	$experts_featured = get_field('experts_featured','option');
	$banner_blog = get_field('expert_analysis_banner','option');
...
<!-- <div id="stickie-animate"></div> -->
	...
	<!-- FILTER SECTION -->
	<div class="filter filter-research" data-archi="<?= 'experts'; ?>">
		<div class="row">
	...
	<?php if ( $experts_featured ) : ?>
		<div class="destacada">
			<div class="row">
				<?php
					foreach ( $experts_featured as $post) :

						setup_postdata( $post );         
						$taxissue = get_the_term_list( $post->ID, 'category-issue', '<span>', ', </span><span>', '</span>' );
						$taxtype = get_the_term_list( $post->ID, 'category-state', '<span>', ', </span><span>', '</span>' );          
				?>
