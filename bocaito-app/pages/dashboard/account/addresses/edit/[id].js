...
const EditAddress = () => {
	...
	const { edit, getAddress } = useAddress()
	...
	const { street, city, state, postcode, country, errors, updated, isEdited } = address

	const saveAddress = event => {
		event.preventDefault()

		edit({ address, dispatch })
	}

	useEffect(() => {
		getAddress({ id: router.query.id, dispatch })
	}, [])
		...
		<>
			<div className='flex items-center justify-between mb-4'>
				<div className='text-2xl font-semibold'>
					Edit <span className='text-slate-400 dark:text-zinc-400 ml-1'>{street}</span>
				</div>
			</div>
			...
			<div className='flex flex-col md:flex-row bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-700 rounded-xl overflow-hidden'>
				<div className='w-full md:w-1/4 border-r border-slate-200/80 dark:border-zinc-700 relative'>
					...
					<div className='w-full px-4 py-3 bg-slate-50 dark:bg-zinc-800 absolute bottom-0 left-0 '>
			...
			<AnimatePresence>
				{isEdited && (
					<SuccessNotification title='Success' message='Your address has been edited!' />
				)}
			</AnimatePresence>
		</>
	)
