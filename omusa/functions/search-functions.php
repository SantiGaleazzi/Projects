...
function template_chooser($template){
	...
	if( $wp_query->is_search) {
  		return locate_template('searchpage.php');  //  redirect to archive-search.php
  	}
...
function add_custom_post_types( $query ) {
    if ( $query->is_main_query() && $query->is_search() && ! is_admin() ) {
        $query->set( 'post_type', array('page', 'post', 'stories','continents','short-term','long-term','videos' ) );
    }
}
