			...
			<?php
				...
				?>
					<div id="video-stories-data" class="stories_found flex flex-wrap flex-col sm:flex-row mb-10 filter-result">
						...
						<?php while ( $video_stories_query->have_posts() ) : $video_stories_query->the_post();
							...
							?>
								...
								<?php if ( $counter == 1 ) : ?>
									...
									<div class="w-full">
										...
										<div class="sm:w-1/2 lg:w-1/3 text-center my-6 mx-auto">
											<a href="https://give.omusa.org/om/ukraine-relief?sc=US2203WBLBUK&mc=ukraine-video-updates" target="_blank" class="w-full text-center text-lg text-white-pure font-black leading-none px-4 py-5 bg-red-500 inline-block">Send help for Ukraine now!</a>
										</div>
									</div>
								<?php else : ?>
